...
describe("Car Page E2E Tests", function () {
    ...
    it("Should load car page when home page car card clicked", async () => {
        const allH5 = await driver.wait(until.elementsLocated(By.css("h5")), 20000);
        ...
        await driver.wait(until.urlContains('/car'), 20000);
        const url = await driver.getCurrentUrl();
        expect(url).to.include('/car');

        const carMakeModelH2 = await driver.wait(until.elementLocated(By.xpath("//h2[1]")), 20000);
        const carMakeModelH2Text = await carMakeModelH2.getText();

        const carMakeModelH1 = await driver.wait(until.elementLocated(By.xpath("//h1[1]")), 20000);
