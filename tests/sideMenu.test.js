...
describe("Side Menu E2E Tests", function () {
    ...
    it("Should open the account page", async () => {
        ...
        await driver.wait(until.elementLocated(By.css('a[href="/account"]')), 20000).click();

        await driver.wait(until.urlContains('/account'), 20000);
        const url = await driver.getCurrentUrl();
        expect(url).to.include('/account');

        const username = await driver.wait(until.elementLocated(By.id("username")), 20000);
        const usernameText = await username.getText();
        expect(usernameText).to.equal("unco");

        const email = await driver.wait(until.elementLocated(By.id("email_address")), 20000);
    ...
    it("Should open the car registration page", async () => {
        ...
        const header = await driver.wait(until.elementLocated(By.xpath("//h1[1]")), 20000);
    ...
    it ("Should open the home page", async () => {
        await driver.wait(until.elementLocated(By.id("menuBtn")), 20000).click();
        await driver.sleep(500);
        await driver.wait(until.elementLocated(By.css('a[href="/home"]')), 20000).click();
