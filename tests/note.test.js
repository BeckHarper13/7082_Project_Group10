...
describe("Edit Note E2E Tests", function () {
    ...
    it("Should create a new note", async () => {
        ...
        const notesAccordionToggle = await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Notes')]")), 50000);
        await notesAccordionToggle.click();
        await driver.sleep(500); // allow animation to finish

        const editNotesBtn = await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Edit Notes')]")), 50000);
        await editNotesBtn.click();
        await driver.sleep(500); // allow animation to finish

        const editNotesInput = await driver.wait(until.elementLocated(By.id("notesTextarea")), 50000);
        await editNotesInput.clear();
        await editNotesInput.sendKeys(note);
        await driver.wait(until.elementLocated(By.id("saveNoteBtn")), 50000).click();

        const noteDisplay = await driver.wait(until.elementLocated(By.id("notesDisplay")), 50000);
    ...
    it("Should display the note on the home page", async () => {
        ...
        const smallNote = await driver.wait(until.elementLocated(By.id("note")), 50000);
