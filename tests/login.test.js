...
describe("Login Page E2E Test", function () {
    ...
    it("Should logout the user", async () => {
        ...
        const logoutForm = await driver.wait(until.elementLocated(By.css('form[action="/logout"]')), 50000);
        const logoutBtn = await logoutForm.findElement(By.css("button"));
        await logoutBtn.click();

        await driver.wait(until.urlContains('http://localhost:3000'), 50000);
        const url = await driver.getCurrentUrl();
        expect(url).to.contain('http://localhost:3000');

        const loginLink = await driver.wait(until.elementLocated(By.xpath("//a[contains(text(), 'Login')]")), 50000);
        const loginTxt = await loginLink.getText();
        expect(loginTxt).to.equal("Login");

        const signup = await driver.wait(until.elementLocated(By.xpath("//a[contains(text(), 'Sign Up')]")), 50000);
